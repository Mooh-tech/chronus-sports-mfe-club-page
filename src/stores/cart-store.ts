...
const calculateShipping = async (cep: string) => {
  ...
  try {
    ...
    const response = await fetch(`${MELHOR_ENVIO_CONFIG.baseUrl}/shipment/calculate`, {
      method: 'POST',
      headers: {
    ...
    const validOptions = processedOptions.filter(option => 
      option.price > 0 && option.delivery_time > 0
    );
  ...
  const processCheckout = async () => {
    ...
    // Verificar se há opção de entrega selecionada
    ...
    loading.value = true;
    try {
      // Aqui você faria a requisição para processar o pagamento
  ...
  // Validar logradouro
  if (!shippingAddress.value.logradouro || shippingAddress.value.logradouro.trim() === '') {
    errors.push('Logradouro é obrigatório')
  }
  
  // Validar número
  if (!shippingAddress.value.numero || shippingAddress.value.numero.trim() === '') {
    errors.push('Número é obrigatório')
  }
  
  // Validar bairro
  if (!shippingAddress.value.bairro || shippingAddress.value.bairro.trim() === '') {
    errors.push('Bairro é obrigatório')
  }
  
  // Validar cidade
  if (!shippingAddress.value.cidade || shippingAddress.value.cidade.trim() === '') {
